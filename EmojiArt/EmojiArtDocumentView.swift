...
struct EmojiArtDocumentView: View {
    ...
    var documentBody: some View {
        GeometryReader { geometry in
            //geometry reader is needed to get the positions and dimensions of the background and emojis
            ZStack {
                Color.white.overlay(
            ...
            .clipped() //prevent view going outside its limits
            .onDrop(of: [.plainText, .url, .image], isTargeted: nil) {
                providers, location in
                return drop(providers: providers, at: location, in: geometry)
            }
            .gesture(panGesture().simultaneously(with: zoomGesture()))
          //if more than one gesture is added to the same view we need to use simultaneously(with)
          // or exclusively(with)
        }
